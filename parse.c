...
char *key_words[] = {"mov", "cmp",  "add",  "sub",    "not",   "clr",   "lea",
                     "inc", "dec",  "jmp",  "bne",    "red",   "prn",   "jsr",
                     "rts", "stop", "r1",   "r2",     "r3",    "r4",    "r5",
                     "r6",  "r7",   "data", "string", "entry", "extern"};
...
int parse_code(char *tok, char *line, int *parse, unsigned int line_index,
               char *fname) {
  int i = 0;
...
int parse_data(char *tok, int data_type, int *parse, unsigned int line_index,
               char *fname) {
  if (data_type) {
    parse += (int)*tok;
  } else {
